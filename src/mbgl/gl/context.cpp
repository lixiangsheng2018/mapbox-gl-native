...
std::unique_ptr<const gfx::RenderbufferResource>
...
void bindDepthStencilRenderbuffer(
    ...
    auto& depthStencilResource = static_cast<const gl::RenderbufferResource&>(*depthStencil.resource);
...
Context::createFramebuffer(const gfx::Renderbuffer<gfx::RenderbufferPixelType::RGBA>& color,
    ...
    auto& colorResource = static_cast<const gl::RenderbufferResource&>(*color.resource);
...
Context::createFramebuffer(const gfx::Texture& color,
    ...
    auto& depthResource = static_cast<const gl::RenderbufferResource&>(*depth.resource);
    MBGL_CHECK_ERROR(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER,
                                               depthResource.renderbuffer));
    checkFramebuffer();
    return { depth.size, std::move(fbo) };
}
